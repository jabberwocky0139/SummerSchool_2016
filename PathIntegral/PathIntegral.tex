...
\begin{eqnarray}
  ...
  G(x, -i\tau; x_0, 0) &=& \qty(\frac{1}{2\pi})^{N+1}\int dx_1 dx_2\cdots dx_N e^{-\int_0^{\tau}d\tau H(\tau)}\label{prob10_2}
\end{eqnarray}\\
...
\begin{eqnarray}
  \int d\tau H(\tau) &\simeq& \sum_j\epsilon E_j = \epsilon W(\{x_j\})\\
...
\subsection{処方箋}
この問題に関する処方箋はRubinの本\footnote{Rubin H. Landau, {\it Computational Physics} 翻訳版 (1997) pp. 313-317}にあるのでこれを参考勉強するのがいいと思います. が, 翻訳があまり良くないのかそもそも原文の内容が丁寧でないためか, 理解するのはなかなか難しいと思います. 理解のための重要なポイントは
\begin{itemize}
...
\item WKB近似で経路積分を再定式化(去年那くんがやった)
\item ちょっと異なるポテンシャルをかけてみる
\item 高次元系への拡張
