...
\begin{eqnarray}
  i\hbar\partial_tf_W(q, p, t) &=& \int_{-\infty}^{\infty} d\overline{q}_{12} \bra{q_1}i\hbar\partial_t\qty(\ket{\psi(t)}\bra{\psi(t)})\ket{q_2}e^{-ip\overline{q}_{12}/\hbar}\\
...
\begin{eqnarray}
  \partial f_W(q, p, t) = \qty(q\partial_p - p\partial_q)f_W(q, p, t)
\end{eqnarray}
...
\begin{eqnarray}
  %\nonumber  f(t_{n+1}) = \qty(k_1q_l + k_2)f(p_{m+1}) - \qty(k_1p_m + k_2)f(q_{l+1}) + k_2\qty(f(p_{m-1}) - f(q_{l-1})) + \qty(-k_1q_l + k_1p_m + 1)f\\
...
\item $f(q_l, p_m, t_n) \rightarrow f$, $f(q_{l+1}, p_m, t_n) \rightarrow f(q_{l+1})$などの表記を用いている.
  \item $h_t/h_{qp} = k_1$, $h_t/4h_{qp}^2 = k_2$.
\end{itemize}
これで数値計算が可能な形になった.
\subsubsection{初期条件}
初期関数として
