...
\end{eqnarray}
とすることもできる. ここでは$q_{12} = (q_1 + q_2)/2$, $\overline{q}_{12} = q_1 - q_2$である. 重心座標と相対座標に分ける変数変換は便利なのでよく使われる. 
\begin{comment}
...
\end{eqnarray}
これを並進変換$e^{a\partial_q}e^{pq} = e^{p(q+a)}$より
\begin{eqnarray}
  \nonumber\frac{1}{(2\pi\hbar)^2}\int ds dq' dp' dp'' &&e^{ips/\hbar}e^{-ip'\qty(q-q'-\frac{s}{2})/\hbar}e^{-ip''\qty(q'-q-\frac{s}{2})/\hbar}\\
...
\end{eqnarray}
$e^{\frac{i\hbar}{2}\qty(\overleftarrow{\partial}_q\overrightarrow{\partial}_{p}-\overleftarrow{\partial}_{p}\overrightarrow{\partial}_{q})} = e^{\frac{i\hbar}{2}\Lambda}$とおくと,
\begin{eqnarray}
...
\begin{eqnarray}
  ...
  &=& \{\{H_W, f_W\}\}
\end{eqnarray}
これをMoyal括弧とか一般化Poisson括弧とか呼んだりする.
\section{具体的な問題}
...
\subsection{数値計算}
\subsubsection{差分化}
今回は簡単のため, 普通に差分化する. 差分化したMoyal方程式は
\begin{eqnarray}
...
\subsection{数値解析}
\fbox{問4} $t, q, p$について(\ref{wigner-moyal2})を中央差分で差分化せよ.\\

\fbox{問5} 位相空間上のWigner関数の時間発展を数値計算し, Gnuplotで可視化せよ. 
...
\item $\Delta q\cdot\Delta p\geq \hbar$の確認
\end{itemize}

\fbox{2} 位相空間ではなくて$q$空間における動きを追う(Schr\"odinger方程式を数値計算する).\\

\fbox{3} 差分法はダサいのでSymplectic法で攻める.\\

\fbox{4} むしろ調和振動子くらいならWigner関数の時間発展も解析的に追えるんじゃね？\\

\fbox{5} コヒーレント状態・コヒーレント変換に関する考察. \\

\fbox{6} Wigner関数の負の値に関する考察.\\
